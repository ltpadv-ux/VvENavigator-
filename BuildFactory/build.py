...
VERSION="3.5.0"; ROOT=Path(__file__).resolve().parent; CONFIG=ROOT/"config"/"navigator.json"
...
def cmd_doctor():
 files=["decision_intelligence.py","executive_cockpit.py","executive_reporting.py","publication_engine.py","document_renderer.py","artifact_generator.py","enterprise_core.py","release_manifest.py","health_engine.py","policy_engine.py","portfolio_engine.py","forecast_engine.py","stress_engine.py","optimization_engine.py","strategy_optimizer.py","recommendation_engine.py","mjop_engine.py","finance_engine.py","financial_cockpit.py","risk_engine.py","decision_engine.py","governance_engine.py","action_engine.py","audit_engine.py","compliance_engine.py","release_engine.py","dashboard_engine.py","report_engine.py","datahub.py","export_engine.py","excel_master.py","navigator_mvp.py","scenario_engine.py","practice_dataset.py"]
 checks={"config":CONFIG.exists(),**{f[:-3]:(ROOT/"src"/f).exists() for f in files},"standard_policy":(ROOT/"config"/"policies"/"standard_vve.json").exists(),"sample_vve_34":(ROOT/"data"/"sample_vve_34.json").exists()}
 for n,ok in checks.items(): print(f"[{'OK' if ok else 'FAIL'}] {n}")
 return 0 if all(checks.values()) else 1
def cmd_status():
 c=load_config(); keys=["project","decision_intelligence","executive_cockpit","executive_reporting","publication","document_rendering","artifact_generation","enterprise","health","policy","portfolio","forecast","stress_test","optimization","strategy_optimizer","recommendation","modules"]

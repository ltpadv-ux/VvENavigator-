...
def main()->int:
 p=argparse.ArgumentParser(description="VvE Navigator 4.4 Production Verification & Release Control Center")
 ...
 out=Path(a.output); out.mkdir(parents=True,exist_ok=True); report_path=out/"production-verification-report.json"; control_path=out/"release-control-center.json"; report_path.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8"); control_path.write_text(json.dumps(control,ensure_ascii=False,indent=2),encoding="utf-8")
 print(json.dumps({**report,"verification_report":str(report_path),"control_center_report":str(control_path)},ensure_ascii=False,indent=2)); return 0 if control.get("status") in {"GROEN","HERSTELD"} else 2

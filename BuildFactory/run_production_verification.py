...
def main()->int:
 p=argparse.ArgumentParser(description="VvE Navigator 4.8 Production Verification, SLA & Continuous Improvement Backlog")
 p.add_argument("dataset",nargs="?",default=str(ROOT/"data"/"sample_vve_34.json")); p.add_argument("--output",default=str(ROOT/"artifacts"/"verification")); p.add_argument("--horizon",type=int,default=30); p.add_argument("--self-heal",action="store_true",default=True); p.add_argument("--sla",type=float,default=95.0)
 ...
 improvement=analyze_sla_root_causes(history,[final_diagnostics]); backlog_path=out/"continuous-improvement-backlog.json"; backlog=update_backlog_file(backlog_path,improvement)
 report["trend_monitor"]=trend; report["reliability_sla"]=sla; report["sla_improvement"]=improvement; report["improvement_backlog"]=backlog
 report_path=out/"production-verification-report.json"; control_path=out/"release-control-center.json"; trend_path=out/"release-trend-monitor.json"; sla_path=out/"reliability-sla-monitor.json"; improvement_path=out/"sla-improvement-report.json"
 report_path.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8"); control_path.write_text(json.dumps(control,ensure_ascii=False,indent=2),encoding="utf-8"); trend_path.write_text(json.dumps(trend,ensure_ascii=False,indent=2),encoding="utf-8"); sla_path.write_text(json.dumps(sla,ensure_ascii=False,indent=2),encoding="utf-8"); improvement_path.write_text(json.dumps(improvement,ensure_ascii=False,indent=2),encoding="utf-8")
 print(json.dumps({**report,"verification_report":str(report_path),"control_center_report":str(control_path),"release_history":str(history_path),"trend_report":str(trend_path),"sla_report":str(sla_path),"improvement_report":str(improvement_path),"improvement_backlog":str(backlog_path)},ensure_ascii=False,indent=2)); return 0 if control.get("status") in {"GROEN","HERSTELD"} and sla.get("compliant") else 2

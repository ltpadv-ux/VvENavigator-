...
def main()->int:
 ...
 scorecard=build_strategy_execution_scorecard(strategy,report); report['strategy_execution_scorecard']=scorecard
 reports={"production-verification-report.json":report,"strategy-execution-scorecard.json":scorecard,"strategy-execution-dashboard.json":{"decision_id":scorecard.get('decision_id',''),"selected_scenario":scorecard.get('selected_scenario',''),"status":scorecard['status'],"score":scorecard['score'],"on_track_count":scorecard.get('on_track_count',0),"off_track_count":scorecard.get('off_track_count',0),"off_track_domains":scorecard.get('off_track_domains',[]),"next_action":scorecard['next_action']},"governance-control-tower.json":tower,"executive-risk-radar.json":radar,"strategic-scenario-radar.json":scenario,"scenario-strategy-dashboard.json":{"status":strategy['status'],"selected_scenario":strategy['selected_scenario'],"decision":strategy['decision'],"deviation":strategy['deviation'],"next_action":strategy['next_action']},"release-control-center.json":control,"reliability-sla-monitor.json":sla,"governance-dashboard.json":register["dashboard"],"alv-decision-dashboard.json":{"status":alv["status"],"item_count":alv["item_count"],"ready_for_alv":alv["ready_for_alv"],"decided_count":alv["decided_count"],"next_action":alv["next_action"]},"alv-mandate-dashboard.json":{"status":governed_mandates["status"],"mandate_count":governed_mandates["mandate_count"],"open_count":governed_mandates["open_count"],"total_budget":governed_mandates["total_budget"],"total_spent":governed_mandates["total_spent"],"budget_remaining":governed_mandates["budget_remaining"],"compliance_status":compliance["status"],"forecast_status":forecast["status"],"next_action":effectiveness["next_action"]}}
 for name,payload in reports.items(): (out/name).write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
 print(json.dumps({**report,"strategy_execution_scorecard_file":str(out/'strategy-execution-scorecard.json')},ensure_ascii=False,indent=2)); return 0 if tower['overall_status']!='ROOD' and scorecard['status']!='BUITEN KOERS' and autonomous.get("cycle_status")=="AUTONOOM GROEN" and not compliance.get("human_escalation_required") else 2

...
def main()->int:
 p=argparse.ArgumentParser(description="VvE Navigator 5.1 Governance Dashboard & Decision Register"); p.add_argument("dataset",nargs="?",default=str(ROOT/"data"/"sample_vve_34.json")); p.add_argument("--output",default=str(ROOT/"artifacts"/"verification")); p.add_argument("--horizon",type=int,default=30); p.add_argument("--self-heal",action="store_true",default=True); p.add_argument("--sla",type=float,default=95.0); p.add_argument("--closure-runs",type=int,default=3); a=p.parse_args()
 ...
 reports={"production-verification-report.json":report,"release-control-center.json":control,"release-trend-monitor.json":trend,"reliability-sla-monitor.json":sla,"sla-improvement-report.json":improvement,"improvement-governance-report.json":governance,"autonomous-governance-report.json":autonomous,"governance-dashboard.json":register["dashboard"]}
 for name,payload in reports.items(): (out/name).write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
 print(json.dumps({**report,"release_history":str(history_path),"improvement_backlog_file":str(backlog_path),"governance_decision_register_file":str(register_path)},ensure_ascii=False,indent=2)); return 0 if autonomous.get("cycle_status")=="AUTONOOM GROEN" else 2

...
def main()->int:
 p=argparse.ArgumentParser(description="VvE Navigator 4.6 Production Verification, History & SLA Monitor")
 ...
 out=Path(a.output); out.mkdir(parents=True,exist_ok=True); history_path=out/"release-history.json"; trend=append_history(history_path,report); sla=evaluate_reliability_sla(trend,minimum_reliability=a.sla,max_blocked_recent=1); report["trend_monitor"]=trend; report["reliability_sla"]=sla
 report_path=out/"production-verification-report.json"; control_path=out/"release-control-center.json"; trend_path=out/"release-trend-monitor.json"; sla_path=out/"reliability-sla-monitor.json"
 report_path.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8"); control_path.write_text(json.dumps(control,ensure_ascii=False,indent=2),encoding="utf-8"); trend_path.write_text(json.dumps(trend,ensure_ascii=False,indent=2),encoding="utf-8"); sla_path.write_text(json.dumps(sla,ensure_ascii=False,indent=2),encoding="utf-8")
 print(json.dumps({**report,"verification_report":str(report_path),"control_center_report":str(control_path),"release_history":str(history_path),"trend_report":str(trend_path),"sla_report":str(sla_path)},ensure_ascii=False,indent=2)); return 0 if control.get("status") in {"GROEN","HERSTELD"} and sla.get("compliant") else 2

...
def main()->int:
 ...
 scorecard=build_strategy_execution_scorecard(strategy,report); report['strategy_execution_scorecard']=scorecard; intervention=build_strategy_interventions(scorecard,report); report['strategy_intervention_engine']=intervention; impact=simulate_intervention_impacts(intervention,report,discount_rate=a.discount_rate); report['intervention_impact_simulator']=impact
 reports={"production-verification-report.json":report,"strategy-execution-scorecard.json":scorecard,"strategy-intervention-report.json":intervention,"intervention-impact-report.json":impact,"intervention-impact-dashboard.json":{"simulation_count":impact['simulation_count'],"discount_rate":impact['discount_rate'],"horizons_years":impact['horizons_years'],"simulations":impact['simulations'],"human_decision_required":impact['human_decision_required'],"next_action":impact['next_action']},"governance-control-tower.json":tower,"executive-risk-radar.json":radar,"strategic-scenario-radar.json":scenario,"scenario-strategy-dashboard.json":{"status":strategy['status'],"selected_scenario":strategy['selected_scenario'],"decision":strategy['decision'],"deviation":strategy['deviation'],"next_action":strategy['next_action']},"release-control-center.json":control,"reliability-sla-monitor.json":sla,"governance-dashboard.json":register["dashboard"]}
 for name,payload in reports.items(): (out/name).write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
 print(json.dumps({**report,"intervention_impact_file":str(out/'intervention-impact-report.json')},ensure_ascii=False,indent=2)); return 0 if tower['overall_status']!='ROOD' and scorecard['status']!='BUITEN KOERS' and autonomous.get("cycle_status")=="AUTONOOM GROEN" and not compliance.get("human_escalation_required") else 2

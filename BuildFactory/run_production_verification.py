...
def main()->int:
 ...
 amendment_result=apply_mandate_amendments(governed_mandates,amendments,existing_amendment_history); governed_mandates=amendment_result["mandates"]; compliance=evaluate_mandate_compliance(governed_mandates); governed_mandates={**governed_mandates,"mandates":compliance["mandates"],"compliance_status":compliance["status"]}; forecast=forecast_mandates(governed_mandates,warning_days=a.mandate_warning_days); governed_mandates["forecast_status"]=forecast["status"]; governed_mandates["corrective_action_status"]=optimizer["status"]; governed_mandates["corrective_approval_status"]=corrective["status"]; governed_mandates["approved_amendments"]=amendments; governed_mandates["amendment_status"]=amendment_result["status"]; mandate_path.write_text(json.dumps(governed_mandates,ensure_ascii=False,indent=2),encoding="utf-8"); amendment_path.write_text(json.dumps(amendment_result,ensure_ascii=False,indent=2),encoding="utf-8")
 report.update(alv_execution_mandates=governed_mandates,mandate_compliance_before=compliance_before,mandate_forecast_before=forecast_before,mandate_compliance=compliance,mandate_forecast=forecast,corrective_action_optimizer=optimizer,corrective_decision_workflow=corrective,approved_mandate_amendments=amendments,mandate_amendment=amendment_result)
 reports={"production-verification-report.json":report,"release-control-center.json":control,"release-trend-monitor.json":trend,"reliability-sla-monitor.json":sla,"sla-improvement-report.json":improvement,"improvement-governance-report.json":governance,"autonomous-governance-report.json":autonomous,"governance-dashboard.json":register["dashboard"],"alv-decision-dashboard.json":{"status":alv["status"],"item_count":alv["item_count"],"ready_for_alv":alv["ready_for_alv"],"decided_count":alv["decided_count"],"next_action":alv["next_action"]},"alv-mandate-dashboard.json":{"status":governed_mandates["status"],"mandate_count":governed_mandates["mandate_count"],"open_count":governed_mandates["open_count"],"total_budget":governed_mandates["total_budget"],"total_spent":governed_mandates["total_spent"],"budget_remaining":governed_mandates["budget_remaining"],"compliance_status":compliance["status"],"forecast_status":forecast["status"],"corrective_action_status":optimizer["status"],"corrective_approval_status":corrective["status"],"amendment_status":amendment_result["status"],"pending_corrective_decisions":corrective["pending_count"],"next_action":amendment_result["next_action"] if amendment_result["applied_count"] else (corrective["next_action"] if corrective["pending_count"] else compliance["next_action"])},"mandate-compliance-report.json":compliance,"mandate-forecast-report.json":forecast,"corrective-action-report.json":optimizer,"corrective-decision-dashboard.json":{"status":corrective["status"],"decision_count":corrective["decision_count"],"approved_count":corrective["approved_count"],"pending_count":corrective["pending_count"],"next_action":corrective["next_action"]},"mandate-amendment-report.json":{"status":amendment_result["status"],"applied_count":amendment_result["applied_count"],"next_action":amendment_result["next_action"]}}
 for name,payload in reports.items(): (out/name).write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
 print(json.dumps({**report,"mandate_amendment_history_file":str(amendment_path)},ensure_ascii=False,indent=2)); return 0 if autonomous.get("cycle_status")=="AUTONOOM GROEN" and not compliance.get("human_escalation_required") else 2
